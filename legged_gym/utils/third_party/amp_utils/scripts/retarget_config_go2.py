...
URDF_FILENAME = f"{AMP_UTILS_DIR}/models/go2/urdf/go2.urdf"
OUTPUT_DIR = f"{AMP_UTILS_DIR}/motion_files/mocap_motions_go2/"

REF_POS_SCALE = 0.95 # 缩放系数,如果遇到关节限位异常，尝试将此数变小
INIT_POS = np.array([0, 0, 0.32]) # go2
INIT_ROT = np.array([0, 0, 0, 1.0])

# SIM_TOE_JOINT_IDS = [8, 14, 20, 26]
# SIM_HIP_JOINT_IDS = [3, 9, 15, 21]
SIM_TOE_JOINT_IDS = [13, 7, 25, 19]
SIM_HIP_JOINT_IDS = [8, 2, 20, 14]
...
DEFAULT_JOINT_POSE = np.array([0.1, 0.5, -1.0,
                               0.1, 0.5, -1.0,
                               -0.1, 0.5, -1.0,
                               -0.1, 0.5, -1.0])
JOINT_DAMPING = [0.01, 0.05, 0.01,
                 0.01, 0.05, 0.01,
                 0.01, 0.05, 0.01,
                 0.01, 0.05, 0.01]
