...
from legged_gym.utils.third_party.amp_utils.kinematics.urdf import build_serial_chain_from_urdf
from legged_gym.utils.third_party.rsl_rl_amp.datasets import pose3d
...
import retarget_config_h1 as config
...
chain_foot_fl = build_serial_chain_from_urdf(
    open(config.URDF_FILENAME, 'rb').read(), config.FL_FOOT_NAME, config.TORSO_NAME
)
chain_foot_fr = build_serial_chain_from_urdf(
    open(config.URDF_FILENAME, 'rb').read(), config.FR_FOOT_NAME, config.TORSO_NAME
)
...
def retarget_pose(robot, default_pose, ref_joint_pos):
    ...
    tar_toe_pos_local = np.squeeze(
        np.concatenate(
            [
                ...
                chain_foot_rr.forward_kinematics(joint_pose[5:10]).get_matrix()[
                    :, :3, 3
                ],
            ],
            axis=-1,
    ...
    # Create tar_toe_pos_local as a zero vector with the same size
    # tar_toe_pos_local = np.zeros(TAR_TOE_POS_LOCAL_SIZE)
